...
var env = /* @__PURE__ */ __name((c, runtime) => {
...
var generateAuthToken = /* @__PURE__ */ __name(async (uid, email, jwtSecret) => {
  const expiration = Math.floor(Date.now() / 1e3) + 60 * 60 * 24 * 7;
  return sign2({ uid, email, exp: expiration }, jwtSecret);
}, "generateAuthToken");
...
var authMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  try {
    ...
    return c.json({ error: "Session expir\xE9e ou invalide. Veuillez vous reconnecter." }, 401);
  }
}, "authMiddleware");
var getFirestoreUrl = /* @__PURE__ */ __name((projectId, docPath) => `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/${docPath}`, "getFirestoreUrl");
var firestoreHeaders = /* @__PURE__ */ __name((token) => ({
...
async function getAccessToken(serviceAccount) {
  ...
  const jwt2 = await sign2(payload, serviceAccount.private_key, "RS256");
  ...
  if (!response.ok) {
    ...
    console.error("Erreur d'obtention du token d'acc\xE8s:", response.status, errorBody);
    throw new Error("Impossible d'obtenir le token d'acc\xE8s pour l'API Firestore.");
  }
  const data = await response.json();
...
app.post("/api/register", async (c) => {
  ...
  const { FIREBASE_SERVICE_ACCOUNT, JWT_SECRET, FIREBASE_PROJECT_ID } = env(c);
  if (!name || !email || !password) {
    return c.json({ error: "Nom, email et mot de passe requis" }, 400);
  }
  try {
    const serviceAccount = JSON.parse(FIREBASE_SERVICE_ACCOUNT);
    const accessToken = await getAccessToken(serviceAccount);
    const userDocUrl = getFirestoreUrl(FIREBASE_PROJECT_ID, `users/${email}`);
    const userCheckResponse = await fetch(userDocUrl, {
      method: "GET",
    ...
    await fetch(userDocUrl, {
      ...
      body: JSON.stringify({
        fields: {
          ...
          isSellerVerified: { booleanValue: false },
          createdAt: { timestampValue: (/* @__PURE__ */ new Date()).toISOString() }
        }
      })
    });
    const token = await generateAuthToken(email, email, JWT_SECRET);
    return c.json({
      message: "Inscription r\xE9ussie",
      token,
      user: { uid: email, email, name }
    });
  } catch (error) {
...
app.post("/api/login", async (c) => {
  ...
  try {
    ...
    const passwordMatch = await comparePassword(password, fields.passwordHash.stringValue);
    if (!passwordMatch) {
      return c.json({ error: "Email ou mot de passe incorrect" }, 401);
    }
    const token = await generateAuthToken(email, email, JWT_SECRET);
    return c.json({
      ...
      user: {
        ...
        isSellerVerified: fields.isSellerVerified.booleanValue
      }
    });
  } catch (error) {
...
app.delete("/api/delete-account", authMiddleware, async (c) => {
  const { FIREBASE_SERVICE_ACCOUNT, FIREBASE_PROJECT_ID } = env(c);
  const payload = c.get("jwtPayload");
  if (!payload) {
    return c.json({ error: "Payload JWT manquant" }, 500);
  }
  const userEmail = payload.uid;
  try {
    const serviceAccount = JSON.parse(FIREBASE_SERVICE_ACCOUNT);
    const accessToken = await getAccessToken(serviceAccount);
    const userDocUrl = getFirestoreUrl(FIREBASE_PROJECT_ID, `users/${userEmail}`);
    const response = await fetch(userDocUrl, {
      method: "DELETE",
      headers: firestoreHeaders(accessToken)
    });
    if (!response.ok && response.status !== 404) {
      throw new Error(`Erreur Firestore lors de la suppression: ${response.status}`);
    }
    return c.json({ message: "Compte supprim\xE9 avec succ\xE8s" });
  } catch (error) {
    console.error("Erreur de suppression:", error);
    return c.json({ error: "\xC9chec de la suppression du compte" }, 500);
  }
});
